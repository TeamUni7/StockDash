...
def update(request):
    if request.method=="POST":
        ...
        name = "xy2"
        file = open(djangoSettings.STATIC_ROOT + '\\dashboard\\' + name + '.json', 'w')
        file.write(json1)
        file.close()
...
def update1(request):                                                   #Actual view for storing the post request from index page in a jason file.
    if(request.method=="POST"):                                         #if the request made was post in form of key1=val1 && key2=val2 && key3=val3.....son on
        ...
        file=open(djangoSettings.STATIC_ROOT+'\\dashboard\\'+name+'.json','w')# open the required file to write values
        file.write(json1)
